...
class LockLaser(Laser):
    def __init__(self, player, screen,color=(214, 212, 71),advance_speed_x=None,
        ...
        self.advance_speed_x = 0.8 + self.rule.stage * 0.3 if advance_speed_x is None else advance_speed_x
        self.speed_x = 0
    ...
    def advance_draw(self,color=(255, 25, 25)):
        if self.player.pos[0] > self.rect.x:
            self.rect.move_ip(self.advance_speed_x, 0)
