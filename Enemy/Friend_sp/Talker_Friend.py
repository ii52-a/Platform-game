...
class TalkerFriend:
    ...
    def check_collision(self):
        ...
        if self.stage==0 and not self.is_talking:
            if self.check_circle_collision((self.bottom.x, self.bottom.y - self.head_radius), self.head_radius + 50,
                                           (self.player.pos[0], self.player.pos[1]), self.player.radius):
                self.is_talking = True

        elif self.stage==1 and not self.is_talking:
            if self.check_circle_collision((self.bottom.x, self.bottom.y - self.body_length-self.leg_length), self.head_radius + 50,
                                           (self.player.pos[0], self.player.pos[1]), self.player.radius):
                self.is_talking = True
